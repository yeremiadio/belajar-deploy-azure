...
import { TChartSensor, TGatewayDevice, TSensorData } from '@/types/api/socket';
...
import { useGetEnviroboxDeviceChartQuery } from '@/stores/enviroboxStore/enviroboxStoreApi';
...
const ChartContainer: FC<Props> = ({ deviceData, gatewayId }) => {
  const [selectedTrend, setSelectedTrend] = useState<string>();
  const [selectedTime, setSelectedTime] = useState<number | null>(
    timeOptions[0]?.value,
  );

  const { data, isLoading } = useGetEnviroboxDeviceChartQuery(
    {
