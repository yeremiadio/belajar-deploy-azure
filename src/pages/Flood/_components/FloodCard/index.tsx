...
import { ESensorFloodEnum, TChartSensor } from '@/types/api/ews/ewsFlood';
import {
  TGatewayDevice,
  TSensorData,
  TSocketGateway,
...
import { FloodInformationCard } from './FloodInformationCard';
...
export const FloodCard: FC<Props & HTMLAttributes<HTMLDivElement>> = ({
  ...
  const [selectedSensor, setSelectedSensor] =
    useState<keyof typeof ESensorFloodEnum>('Water Level');
  ...
  const actionsDropdown = useMemo(() => {
    const filteredChartData = Object.keys(ESensorFloodEnum);
    const mappedDropdown: IMenuDropdownItem[] = filteredChartData.map(
      (item) => {
        return {
          label: convertCamelCaseToCapitalLetter(item),
          onClick: () =>
            setSelectedSensor(item as keyof typeof ESensorFloodEnum),
        };
      },
    );
  ...
  const chartData: ChartData<'line', number[], number> = useMemo(() => {
    const selectedDevice = getSensor(ESensorFloodEnum[selectedSensor]);
    ...
    const chartSensor: TChartSensor | undefined = data?.find(
      (item) => item.sensorcode === ESensorFloodEnum[selectedSensor],
    );
      ...
      <div className="mb-3 flex items-center justify-between border-b border-rs-v2-thunder-blue pb-2">
        ...
        <span
          ...
          onClick={(e) => {
            ...
            navigate(
              gatewayId
                ? ROUTES.ewsFloodGatewayDetail(
                    gatewayId,
                    selectedDevice.id.toString(),
                  )
                : ROUTES.ewsFloodDetail(selectedDevice.id.toString()),
            );
          }}
        >
          See Detail
      ...
      <div className={cn(`flex flex-col gap-y-2 `)}>
        ...
        <div>
          {datasets[0]?.data && datasets[0]?.data.length > 0 ? (
            <TooltipChart
              chartData={chartData}
              unit={getSensor(ESensorFloodEnum[selectedSensor])?.unit ?? ''}
            />
          ) : (
