...
import { ESensorSolidWasteEnum, TWasteDevice } from '@/types/api/wasteMonitoring';
...
const SolidChartCard: FC<Props> = ({deviceIds, deviceList}) => {
  const [searchRanges, setSearchRanges] = useState<TChartRanges>('WEEK');
  const { gatewayId } = useParams<'gatewayId'>();
  const [selectedSensor, setSelectedSensor] = useState<string>('weight');
  
  const SolidWasteOptions = Object.keys(ESensorSolidWasteEnum).map((key) => ({
    label: key,
    value: ESensorSolidWasteEnum[key as keyof typeof ESensorSolidWasteEnum],
  }));  
  ...
  const totalWeightValue = sensorData?.reduce((sum, item) => sum + item.value, 0);
  ...
  return (
    <div className="flex flex-col gap-4 w-full">
      ...
      <div className="flex flex-row justify-between items-center">
        ...
        <span>{totalWeightValue} {unit}</span>
      </div>
